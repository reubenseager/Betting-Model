...
X_train, X_test, y_train, y_test = train_test_split(input_data.drop(columns=["result"], axis=1), input_data["result"], test_size=0.2, shuffle=False) #Setting shuffle to false, again to prevent data leakage
...
#Only selecting features that appear in either of the sets selected by the models
...
joblib.dump(fs_columns, f"{intermediate}/fs_columns_differentials_leak_fix.pkl")
