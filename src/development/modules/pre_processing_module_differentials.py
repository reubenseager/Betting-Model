...
#Going to create a column that gives the percentage of points gained from the possible points.
...
#Calculating the cumulative points for each team up to the current game by season
match_data_all_teams["cumulative_gf"] = match_data_all_teams.groupby(["team_full_name", "season"])["gf"].cumsum()
match_data_all_teams["cumulative_ga"] = match_data_all_teams.groupby(["team_full_name", "season"])["ga"].cumsum()
...
cols_for_rolling = ["gf", "ga", "xg", "xga", "npxg" ,"points", "sh", "poss", "sot", "dist", "perc_points"]
...
#Likely to remove this as it doesn't seem to be chosen by the model and costs us a lot of data.
...
#Just doing these for now, may increase later on
differential_cols = ["gd", "weighted_league_position", "team_elo_points", "perc_points_rolling", "points_rolling", "cumulative_points"]
