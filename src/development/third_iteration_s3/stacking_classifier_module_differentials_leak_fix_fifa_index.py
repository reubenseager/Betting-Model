...
model_studies_location = Path.cwd() / "data" / "intermediate" / "model_studies_differentials_leak_fix_fifa_index"
...
input_data_fs = input_data[[col for col in input_data.columns if col.endswith("_differential") or col.startswith("ip_")] + ["result"]]

#I'm holding back 20% of the data for final model validation and testing
X_train, X_test, y_train, y_test = train_test_split(input_data_fs.drop(columns=["result"], axis=1), input_data_fs["result"], 
                                                    test_size=0.20, shuffle=False) #Setting shuffle to false, again to prevent data leakage
...
#Adjusting the best parameters for the models 
#KNN model
...
if dimred_knn_study_best_params["comp_analysis"] == "LDA":
    ...
    del dimred_knn_study_best_params['comp_analysis']
    del dimred_knn_study_best_params['pca_n_components']
    ...
    del dimred_knn_study_best_params['comp_analysis']
    del dimred_knn_study_best_params['nca_n_components']
else:
...
#GNB model
dimred_gnb_study_best_params = dimred_gnb_study.best_params.copy()

if dimred_gnb_study_best_params["comp_analysis"] == "PCA":
    dimred_gnb_method = PCA(n_components=dimred_gnb_study_best_params["pca_n_components"])
    del dimred_gnb_study_best_params['comp_analysis']
    del dimred_gnb_study_best_params['pca_n_components']

else:
    dimred_gnb_method = "passthrough"
    del dimred_gnb_study_best_params['comp_analysis']
...
level_0_classifiers["dimred_gnb"] = make_pipeline(dimred_gnb_method,
                                                  GaussianNB(**dimred_gnb_study_best_params))
...
#we are going to be using half of the remaining data (10%) to tune the level 1 classifier and the other half to test the model (10%).

#First we need to create the stacked probabilities for the test data
stacked_probabilities_df_test_val = pd.DataFrame()

#Scaling the test data. The same scaler is being used as that was used on the training data
X_test_val_scaled = pd.DataFrame(scaler.transform(X_test), columns=X_test.columns, index = X_test.index)#scaling the training data

for name, classifier in level_0_classifiers.items():
    classifier_ = cp.deepcopy(classifier)
    classifier_.fit(X_train, y_train)

    y_predict_proba = classifier_.predict_proba(X_test_val_scaled)
    ...
    # Add a new column for each class
    for i, class_name in enumerate(class_names):
        stacked_probabilities_df_test_val = pd.concat([stacked_probabilities_df_test_val, pd.DataFrame(y_predict_proba[:, i], columns=[f"{name}_{class_name}_prediction"])], axis=1)
...
stacked_probabilities_df_test_val = stacked_probabilities_df_test_val.assign(index_number = range(len(stacked_probabilities_df_test_val)))
X_test_val_scaled = X_test_val_scaled.assign(index_number = range(len(X_test_val_scaled)))

#Joining the data together
X_test_val_stacked = pd.merge(X_test_val_scaled, stacked_probabilities_df_test_val, on="index_number", how="inner")


x_val, x_test_hold, y_val, y_test_hold = train_test_split(X_test_val_stacked, y_test, test_size=0.5, shuffle=False)
#x_val, x_test_hold, y_val, y_test_hold = train_test_split(X_test_stacked, y_test, test_size=0.5, shuffle=False)

#Creating a time series split for the cross validation
tscv_cl1 = TimeSeriesSplit(gap=0, max_train_size=None, n_splits=8, test_size=None)
...
def objective(trial):
    ...
    f1_macro = cross_val_score(model, x_val, y_val, cv=tscv_cl1, scoring="accuracy").mean()
    return f1_macro
...
level_1_classifier_best_params = study.best_params

level_1_classifier = RandomForestClassifier(**level_1_classifier_best_params, random_state=41)
...
y_hold_pred_stacked = level_1_classifier.predict(x_test_hold.drop(labels=["index_number"], axis=1))

#converting the predictions to a dataframe
y_pred_stacked_df = pd.DataFrame(y_hold_pred_stacked, columns=["prediction"], index=x_test_hold.index)
y_pred_stacked_df.value_counts(normalize=True)
y_test.value_counts(normalize=True)
#classification report
print(classification_report(y_test_hold, y_hold_pred_stacked))
